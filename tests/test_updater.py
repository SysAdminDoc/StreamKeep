...
class UpdaterTests(unittest.TestCase):
    def test_cancelled_update_download_removes_partial_new_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch.object(worker, "_target_path", return_value=str(exe_path)), \
                 mock.patch("streamkeep.updater.urllib.request.urlopen", return_value=_FakeResponse([payload])), \
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch.object(worker, "_target_path", return_value=str(exe_path)), \
                 mock.patch(
                     ...
                     return_value=_FakeResponse([payload], headers={"Content-Length": "not-a-number"}),
                 ), \
                 mock.patch("streamkeep.updater.sys.executable", str(exe_path)), \
                 mock.patch("streamkeep.updater.sys.frozen", True, create=True):
                worker.run()
    ...
    def test_missing_sha256_blocks_update_before_download(self):
        app = QCoreApplication.instance() or QCoreApplication([])
        done_events = []
        with tempfile.TemporaryDirectory() as tmpdir:
            exe_path = Path(tmpdir) / "StreamKeep.exe"
            ...
            with mock.patch.object(worker, "_target_path", return_value=str(exe_path)), \
                 mock.patch("streamkeep.updater.urllib.request.urlopen") as urlopen, \
                 mock.patch("streamkeep.updater.sys.executable", str(exe_path)), \
                 mock.patch("streamkeep.updater.sys.frozen", True, create=True):
                worker.run()
            ...
            self.assertIn("missing SHA-256", done_events[0][1])
            urlopen.assert_not_called()
            self.assertFalse((exe_path.parent / "StreamKeep.exe.new").exists())

    def test_malformed_sha256_blocks_update_before_download(self):
        app = QCoreApplication.instance() or QCoreApplication([])
        done_events = []
        with tempfile.TemporaryDirectory() as tmpdir:
            exe_path = Path(tmpdir) / "StreamKeep.exe"
            ...
            self.assertTrue(done_events)
            self.assertFalse(done_events[0][0])
            self.assertIn("malformed", done_events[0][1])
            urlopen.assert_not_called()
            self.assertFalse((exe_path.parent / "StreamKeep.exe.new").exists())
