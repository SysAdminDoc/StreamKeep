...
class LocalServerTests(unittest.TestCase):
    ...
    def test_status_and_failure_actions_expose_retryable_jobs(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(status_payload["failures"][0]["remediation"]["message"])
            self.assertNotIn("://", json.dumps(status_payload["failures"][0]["remediation"]))
    ...
    def test_authenticated_operations_view_actions_and_export(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(page["rows"][0]["category"], "network")
            self.assertTrue(page["rows"][0]["remediation"]["message"])
            self.assertNotIn("://", json.dumps(page["rows"][0]["remediation"]))
