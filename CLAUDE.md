...
- v4.28.0 — **AI Auto-Highlight Generator (F57).** New `intelligence/highlight.py`: composite scoring from chat spikes + audio peaks + scene density. Top-N with gap. HighlightWorker. History right-click action.
- v4.28.1 — **SponsorBlock Integration (F58).** New `integrations/sponsorblock.py`: SHA256-prefix privacy API, segment fetch, chapter converter.
- v4.28.2 — **Platform Subtitle Download (F59).** New `subtitles.py`: VTT/TTML->SRT converters, ytdlp_sub_args, subtitle discovery + normalization.
- v4.28.3 — **Content Summary via LLM (F60).** New `intelligence/summarize.py`: ollama/Anthropic/OpenAI backends, chunked transcript processing, .summary.md output.
- v4.28.4 — **Smart Thumbnail Generator (F61).** New `intelligence/thumbnail.py`: frame scoring (contrast + edges + faces), text overlay, ThumbnailGenWorker.
- v4.28.5 — **Audio Normalization Profiles (F62).** New `postprocess/normalization.py`: two-pass loudnorm, 4 named profiles (Broadcast/Podcast/YouTube/Streaming). PostProcessor upgraded from single-pass.
- v4.29.0 — **Download Analytics Dashboard (F63).** New Analytics tab (6th tab). QPainter charts: daily bar, platform donut, top channels horizontal bar. Metric cards + date range filter.
- v4.29.1 — **Bandwidth Usage Tracker (F64).** New `bandwidth.py`: singleton tracker with daily SQLite persistence, daily/monthly caps, format helpers.
- v4.29.2 — **Download Integrity Verification (F65).** New `verify.py`: ffprobe-based post-download check. Duration/size/stream validation with ok/warn/fail status.
- v4.29.3 — **Channel Statistics & Growth Trends (F66).** New `channel_stats.py`: log live/offline transitions, aggregate streams/week, avg duration, top games, weekly sparkline data.
- v4.29.4 — **Storage Health Monitor & Disk Alerts (F67).** New `disk_monitor.py`: 30s poll, warning/critical thresholds, auto-pause signal, status bar color coding.
- v4.30.0 — **Upload Destinations (F68).** New `upload/` package: adapter pattern with S3/B2/MinIO (boto3), FTP/SFTP (ftplib/paramiko), WebDAV (HTTP PUT). UploadWorker + test_connection.
- v4.30.1 — **Clip Share via Local Web Gallery (F69).** New `gallery.py`: /gallery grid, /share/{id} player, /media/{id} Range streaming. UUID share IDs, Catppuccin HTML.
- v4.30.2 — **RSS Feed Generator (F70).** New `feed.py`: RSS 2.0 with enclosure tags + itunes:duration. Per-channel + combined feeds, podcast app compatible.
- v4.30.3 — **Recording Notes & Annotations (F71).** New `notes.py`: .notes.md sidecars, auto-template, search_notes() for global search.
- v4.30.4 — **Backup & Restore Wizard (F72).** New `backup.py`: ZIP-based .skbackup with config+DB+tags+notifications. Restore with pre-restore backup. Auto-backup rotation (keep last N).
- v4.31.0 — **Wave 2 Phase 14 complete (F73-F80).**
  - **F73 Custom Accent Color Picker**: 8 preset swatches + custom hex. `apply_accent()` patches CAT palette + rebuilds stylesheet.
  - **F74 Multi-Language (i18n)**: `streamkeep/i18n/` package. `install_translator(lang)`, `available_languages()`. QTranslator loading from .qm files. English fallback.
  - **F75 Layout Density Modes**: compact/cozy/spacious presets in theme.py. `set_density(name)` / `get_density()`.
  - **F76 First-Run Onboarding Wizard**: `ui/onboarding.py` OnboardingWizard dialog. 4 pages: welcome+ffmpeg, output dir, theme, done. Skip All button.
  - **F77 Plugin / Extension SDK**: `plugins.py` PluginManager. Scans `%APPDATA%/StreamKeep/plugins/`, validates plugin.json manifests, importlib loading. Enable/disable per plugin.
  - **F78 Accessibility (High Contrast)**: `CAT_HIGH_CONTRAST` palette (WCAG AAA, 7:1 contrast). Added to THEMES dict.
  - **F79 Encrypted Config Storage**: `secrets.py` with DPAPI (Windows) / base64 fallback. `protect()`/`unprotect()`, `SENSITIVE_FIELDS`, config field migration helpers.
  - **F80 Native OS Notifications**: `native_notify.py` with Windows Toast (windows-toasts/win10toast), Qt fallback. Action button callbacks (open folder). `is_native_available()`.
