...
- [ ] P1 — V56 — Resumed multi-segment downloads never finalize or clear their sidecar on success
  Category: correctness
  Where: `streamkeep/workers/download.py:1706-1723` (end-of-run block), with `streamkeep/ui/main_window_jobs.py:168` and `streamkeep/ui/tabs/download_queue.py:1632-1656` (resume construction) and `streamkeep/resume.py:389` (`merge_completed`).
  Problem: When an interrupted multi-segment (HLS) download is resumed and then runs to full completion, the worker emits neither `all_done` nor clears `.streamkeep_resume.json`, and logs `"[PARTIAL] Some segments failed"`. Because `all_done` is what drives finalization (`download_single.py` `_on_all_done` → metadata save, History insert, post-processing, queue completion), a successfully-resumed recording is never written to History, never post-processed, and its resume sidecar is orphaned — so on the next launch the resume banner re-offers an already-complete download. This triggers on EVERY resume-to-completion, not an edge case.
  Evidence: End-of-run uses strict set equality: `expected = {s[0] for s in self.segments}`; `completed = set(self._resume_state.completed)`; `if expected != completed: all_succeeded = False` (download.py:1711-1714). On resume, `main_window_jobs.py:168` sets `remaining = remaining_segments(state)` and builds the worker with `segments=remaining`; `_attach_resume_to_worker` (download_queue.py:1656) overwrites `state.segments = worker.segments` but does NOT reset `state.completed`; `attach_resume_state` (download.py:175) points `self._resume_state` at that same `state`, whose `.completed` still holds the previously-done indices. As each remaining segment finishes, `_mark_segment_done` → `merge_completed` (resume.py:389) appends to `state.completed`. So with completed `{0,1,2}` and remaining `{3,4,5}`, the final `completed` is `{0,1,2,3,4,5}` while `expected` is `{3,4,5}` → `expected != completed` is always True whenever the pre-resume completed set was non-empty (the whole point of resume). Verified by tracing all four files; grep confirms no test exercises resume-to-completion (`tests/test_download_worker.py` covers only single-segment yt-dlp cancel/keep).
  Fix: Compare subset membership, not equality: `if not expected.issubset(completed): all_succeeded = False` (equivalently `if expected - completed:`). This preserves the v4.31.2 intent (do not clear on genuine partial failure) while treating a superset `completed` as full success.
  Acceptance: A new test builds a resume worker with `state.completed = {0,1,2}` and `segments = [(3,…),(4,…),(5,…)]`, drives all three to success, and asserts `all_done` was emitted, `clear_resume_state` was called, and no `[PARTIAL]` line was logged; a genuine partial-failure resume still keeps the sidecar and does not emit `all_done`.
  Confidence: Verified.
  Effort: S
