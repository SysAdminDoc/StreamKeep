...
1. ~~**`pytest` writes into the production config directory.**~~ **RESOLVED — verified 2026-08-06.** `tests/conftest.py` now calls `paths.bind_config_dir` against a temp directory before importing any stateful module, and `tests/test_config_isolation.py` gates it: six checks assert every module path resolves inside the isolated root and that nothing under the real profile has an mtime newer than the timestamp taken before the first StreamKeep import. The finding below is kept for its reproduction detail; do not re-open it without first running `pytest tests/test_config_isolation.py`.
