...
- Generate contact sheets, chapters, subtitle files, transcripts, highlights, and silence-removed cuts. Transcription keeps WhisperX, faster-whisper, and whisper.cpp as its preferred backends; when a resolved FFmpeg 8.1.2+ build exposes the `whisper` filter, Settings can enable it as a last-resort fallback by pointing to a local whisper.cpp model. Every backend writes the same SRT, VTT, transcript JSON, and auto-chapter sidecars. **Smart thumbnails** are resource-bounded and preserve source artwork. **LLM summaries** are local-first through History, CLI, and the authenticated local API; cloud requests require an exact transcript preview and one-use consent, with optional redaction.
