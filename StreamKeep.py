...
This file is now a thin shim: it bootstraps dependencies, re-exports
the package symbols under their legacy names (so the UI class below
keeps working), and defines the main window. All non-UI code lives in
the `streamkeep/` package next to this file.

Phase 2 will split the UI class into `streamkeep/ui/`. Until then,
`StreamKeep.py` is the runtime entry point.
"""
...
# PyQt6 — UI class below pulls from here directly.
...
# ── Package re-exports ────────────────────────────────────────────────────
# Legacy name aliases so the UI class below (Phase 2 will split it into
# streamkeep/ui/) doesn't need to be touched during the structural move.
...
# Legacy compatibility: the Settings tab used to flip a module-level
# NATIVE_PROXY global here. Phase 2 will rewrite those sites to call
# `_set_native_proxy()` directly, but for now keep the assignments working.
NATIVE_PROXY = ""
...
def main():
    from streamkeep.crash_log import setup_crash_logging as _setup_crash_logging
