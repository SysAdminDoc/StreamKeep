...
def _query_openai_compat(prompt, api_url, api_key, model="gpt-4o-mini"):
...
def _query_anthropic(prompt, api_key, model="claude-sonnet-4-20250514"):
...
def _query_llm(prompt, provider="ollama", model="", api_url="", api_key=""):
    ...
    if provider == "ollama":
        return _query_ollama(prompt, model=model or "llama3")
    elif provider == "anthropic":
        return _query_anthropic(prompt, api_key=api_key, model=model or "claude-sonnet-4-20250514")
...
def summarize_recording(recording_dir, *, provider="ollama", model="",
    ...
    if len(chunks) == 1:
        ...
        summary = _query_llm(prompt, provider, model, api_url, api_key)
    else:
        ...
        for i, chunk in enumerate(chunks):
            ...
            cs = _query_llm(prompt, provider, model, api_url, api_key)
            if cs:
